...
export class StringUtil
{
	...
	public static monetaryFormat(val: number, precision: number = 2): string
	{
		let it = StringUtil.MONETARY_UNITS.lower_bound(val);
		
		if (it.equals(StringUtil.MONETARY_UNITS.end()) == false)
			return StringUtil.numberFormat(val / it.first, precision) + " " + it.second;
		else
			return StringUtil.numberFormat(val, precision);
	}

	private static MONETARY_UNITS: std.TreeMap<number, string> = new std.TreeMap(
	[
		std.make_pair(10000 * 10000, "억"),
		std.make_pair( 1000 * 10000, "천만")
	], std.greater);
}